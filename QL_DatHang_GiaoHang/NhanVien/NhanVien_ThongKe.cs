    ...
    public partial class NhanVien_ThongKe : Form
    {
        ...
        public void Refresh_DataGrigview_Doanhthu()
        {
            try
            {
                ...
                string query = "EXEC XemThongKe_curdate";
        ...
        private void xemDS_Click(object sender, EventArgs e)
        {
            try
            {
                string query = "Select TOP 1000 MA_CUA_HANG , TEN_CUA_HANG , NGUOI_DAI_DIEN , MA_SO_THUE from CUAHANG";
                dataGridView_DoanhThu.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                dataGridView_DoanhThu.DataSource = Dataprovider.Instance.ExecuteQuery(query, new object[] {});
            }
            catch (Exception error)
            {
                MessageBox.Show("lỗi rồi nè !!! " + error.ToString());
            }
        }
    }
}
